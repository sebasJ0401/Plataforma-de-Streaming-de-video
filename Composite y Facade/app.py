...
def register():
    ...
    # ── Facade ────────────────────────────────────────────────────────────────
    # Un solo método reemplaza: validación, Factory, BD, sesión y Bridge.
    # La ruta ya no conoce los subsistemas internos del registro.
    facade = UserRegistrationFacade(db, session, ConsoleNotificationSender())
...
def login():
    ...
    # ── Adapter ───────────────────────────────────────────────────────────────
    # Detecta si el login viene de Google o es local.
    # En ambos casos la ruta usa la misma interfaz: get_email() y get_username().
    auth = get_auth_adapter(data)
    ...
    if auth.is_google():
        # Login con Google: si el usuario no existe se crea automáticamente
        if not user:
            user = UserFactory.create(
                username=auth.get_username(),
                email=email,
                password=email + '_google_auto',
                subscription='free'
            )
            db.session.add(user)
            db.session.commit()
    else:
        # Login local: verifica contraseña con check_password()
        password = data.get('password', '')
...
def upload_video():
    ...
    # ── Bridge ────────────────────────────────────────────────────────────────
    notif = VideoUploadedNotification(ConsoleNotificationSender(), video.title)
    notif.notify(recipient=user.username)
    # ─────────────────────────────────────────────────────────────────────────
...
def upgrade():
    ...
    # ── Bridge ────────────────────────────────────────────────────────────────
    notif = UpgradeNotification(ConsoleNotificationSender(), plan)
    notif.notify(recipient=user.username)
    # ─────────────────────────────────────────────────────────────────────────
...
def update_profile():
    ...
    if 'username' in data:
        ...
        if existing and existing.id != user.id:
            return jsonify(APIResponseBuilder().set_error('Username taken').build()), 400
...
@app.route('/api/video/<int:video_id>/clone', methods=['POST'])
@login_required
def clone_video(video_id):
    """
    Clona un video existente usando el patrón Prototype.
    El clon hereda todos los atributos del original pero es un objeto
    independiente que puede modificarse sin afectar al video original.
    """
    user  = get_current_user()
    video = Video.query.get_or_404(video_id)
    ...
    # ── Prototype ─────────────────────────────────────────────────────────────
    # Se crea el prototipo a partir del video original y se clona.
    # El clon puede recibir un nuevo título y categoría opcionales.
    prototype = VideoPrototype(video)
    cloned    = prototype.clone(
        new_title    = data.get('title'),
        new_category = data.get('category'),
        new_is_premium = data.get('is_premium')
    )
    # ─────────────────────────────────────────────────────────────────────────
    ...
    response = (APIResponseBuilder()
                .set_success(True)
                .set_video_id(cloned.id)
                .set_message(f'Video clonado exitosamente como: {cloned.title}')
                .build())
...
def catalog():
    """
    Devuelve el catálogo completo de videos organizado como árbol
    usando el patrón Composite. Cada categoría es un nodo que agrupa
    sus videos como hojas. Las estadísticas se calculan recursivamente.
    """
    # ── Composite ─────────────────────────────────────────────────────────────
    # build_catalog_from_db() construye el árbol completo desde la BD.
    # El cliente trata igual a CategoryNode y VideoLeaf gracias a
    # la interfaz común CatalogComponent.
    root = build_catalog_from_db()

    def serialize(component):
        """Serializa cualquier CatalogComponent a dict recursivamente."""
        from patterns import CategoryNode, VideoLeaf
        if isinstance(component, VideoLeaf):
            return {
                'type':       'video',
                'name':       component.get_name(),
                'views':      component.get_total_views(),
                'video_count': component.get_video_count(),
            }
        return {
            'type':        'category',
            'name':        component.get_name(),
            'video_count': component.get_video_count(),
            'total_views': component.get_total_views(),
            'children':    [serialize(c) for c in component._children],
        }
    # ─────────────────────────────────────────────────────────────────────────

    response = (APIResponseBuilder()
                .set_success(True)
                .set_message(root.get_info())
                .build())
    response['catalog'] = serialize(root)
    return jsonify(response)
