...
def register():
    ...
    if not username or not email or not password:
        return jsonify(APIResponseBuilder().set_error('Todos los campos son requeridos').build()), 400
    if len(password) < 6:
        return jsonify(APIResponseBuilder().set_error('La contraseña debe tener al menos 6 caracteres').build()), 400
    if User.query.filter_by(username=username).first():
        return jsonify(APIResponseBuilder().set_error('El nombre de usuario ya existe').build()), 400
    if User.query.filter_by(email=email).first():
        return jsonify(APIResponseBuilder().set_error('El email ya está registrado').build()), 400
    ...
    # ── Bridge ────────────────────────────────────────────────────────────────
    # La abstracción RegisterNotification sabe QUÉ notificar.
    # ConsoleNotificationSender sabe CÓMO enviarlo.
    # Ambas jerarquías son independientes — cambiar el canal
    # no requiere modificar la notificación y viceversa.
    notif = RegisterNotification(ConsoleNotificationSender())
    notif.notify(recipient=username)
    # ─────────────────────────────────────────────────────────────────────────
